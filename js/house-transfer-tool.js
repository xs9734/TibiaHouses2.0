...
let house_list = [10006, 10007, 10101, 10102, 10103, 10104, 10112, 10113, 10114, 10121, 10122, 10123, 10124, 10201, 10202,
...
let house_list_counter = 0

function sleep(ms) {
    return new Promise(resolve => setTimeout(resolve, ms));
 }
async function Load_House_List(){
    let server = 'Nefera'; //make this selectable
    for(let i=0 ; i <house_list.length; i++){ //length of array will match the number of loops
        let house_list_link = `https://api.tibiadata.com/v3/house/${server}/${house_list[house_list_counter]}`
        fetch(house_list_link).then(
            function(response){
                ...
                response.json().then(function(data){
                    if(data.house.status.is_transfering == true){ //add a new line to confirm if it is pending or a done deal.
                        let houseinfo = document.createElement("tr")
                        houseinfo.innerHTML=
                            `
                            <td>${data.house.houseid}</td>
                            <td>${data.house.name}</td>
                            <td>${data.house.status.rental.owner}</td>
                            <td>${data.house.status.rental.transfer_receiver}</td>
                            <td>${data.house.status.rental.transfer_price}</td>
                            <td>${data.house.world}</td>
                            <td>${data.house.status.rental.moving_date}</td>
        ...
        house_list_counter++; //go through all houses in house_list[]
        console.log("waiting 1 second.");
        await sleep(100);
        console.log("continuing.");
    }
}
async function Load_House_List_Antica(){
    let server = 'Antica'; //make this selectable
    for(let i=0 ; i <house_list.length; i++){ //length of array will match the number of loops
        let house_list_link = `https://api.tibiadata.com/v3/house/${server}/${house_list[house_list_counter]}`
        fetch(house_list_link).then(
            function(response){
...
async function Load_House_List_Secura(){
    let server = 'Secura'; //make this selectable
    for(let i=0 ; i <house_list.length; i++){ //length of array will match the number of loops
        let house_list_link = `https://api.tibiadata.com/v3/house/${server}/${house_list[house_list_counter]}`
        fetch(house_list_link).then(
            function(response){
